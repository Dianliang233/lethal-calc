...
<template>
  <div class="grid grid-cols-9 pb-6 px-6 gap-6">
    ...
    <div class="col-span-5 grid grid-rows-5 grid-cols-5 w-full gap-2 font-size-4xl font-medium">
      <RndBtn class="font-size-5xl" @click="input('#?\\coloneqq#@')">
        <span v-html="convertLatexToMarkup('\\coloneqq')" />
      </RndBtn>
      <RndBtn @click="input('(')">(</RndBtn>
      <RndBtn @click="input(')')">)</RndBtn>
      <RndBtn @click="emit('memoryAdd')">M+</RndBtn>
      <RndBtn @click="memoryReset">MR</RndBtn>
